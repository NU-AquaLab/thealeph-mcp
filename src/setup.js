#!/usr/bin/env node
/**
 * The Aleph MCP Setup Utility
 * Helps users configure Claude Desktop with The Aleph MCP
 */
...
class SetupUtility {
  ...
  getTheAlephMCPPath() {
    console.log(chalk.blue('🔍 Locating The Aleph MCP installation...\n'));
    
    // Method 1: Check if thealeph-mcp is in PATH and get its location
    try {
      const whichCmd = process.platform === 'win32' ? 'where' : 'which';
      const cmdPath = execSync(`${whichCmd} thealeph-mcp`, { encoding: 'utf8' }).trim().split('\n')[0];
      ...
      if (cmdPath && fs.existsSync(cmdPath)) {
          ...
          try {
            ...
            if (nodeMatch && nodeMatch[1]) {
              ...
              if (fs.existsSync(actualPath)) {
                console.log(chalk.green(`✓ Found thealeph-mcp at: ${actualPath}`));
                return actualPath;
              }
            }
          } catch (e) {
        ...
        console.log(chalk.green(`✓ Found thealeph-mcp at: ${cmdPath}`));
        return cmdPath;
      }
    } catch (error) {
    ...
    try {
      ...
      const possiblePaths = [
        path.join(npmRoot, 'thealeph-mcp', 'bin', 'thealeph-mcp'),
        path.join(npmRoot, '.bin', 'thealeph-mcp'),
        path.join(npmBin, 'thealeph-mcp'),
        path.join(npmRoot, '..', '.bin', 'thealeph-mcp'),
        // Windows specific paths
        path.join(npmRoot, 'thealeph-mcp', 'bin', 'thealeph-mcp.cmd'),
        path.join(npmBin, 'thealeph-mcp.cmd')
      ];
      ...
      for (let checkPath of possiblePaths) {
        ...
        if (fs.existsSync(checkPath)) {
          console.log(chalk.green(`✓ Found thealeph-mcp at: ${checkPath}`));
          return checkPath;
        }
      }
    } catch (error) {
    ...
    const commonPaths = [
      // Unix-like systems
      '/usr/local/lib/node_modules/thealeph-mcp/bin/thealeph-mcp',
      '/usr/lib/node_modules/thealeph-mcp/bin/thealeph-mcp',
      path.join(homeDir, '.npm-global', 'lib', 'node_modules', 'thealeph-mcp', 'bin', 'thealeph-mcp'),
      path.join(homeDir, '.nvm', 'versions', 'node', '*', 'lib', 'node_modules', 'thealeph-mcp', 'bin', 'thealeph-mcp'),
      // Windows
      'C:\\Program Files\\nodejs\\node_modules\\thealeph-mcp\\bin\\thealeph-mcp',
      'C:\\Program Files (x86)\\nodejs\\node_modules\\thealeph-mcp\\bin\\thealeph-mcp',
      path.join(process.env.APPDATA || '', 'npm', 'node_modules', 'thealeph-mcp', 'bin', 'thealeph-mcp')
    ];
    ...
    for (let checkPath of commonPaths) {
      ...
      if (checkPath.includes('*')) {
        try {
          ...
          if (matches.length > 0) {
            console.log(chalk.green(`✓ Found thealeph-mcp at: ${matches[0]}`));
            return matches[0];
          }
        } catch (e) {
    ...
    console.log(chalk.red.bold('❌ Could not find The Aleph MCP installation.\n'));
    console.log(chalk.yellow('Please install it first with:'));
    console.log(chalk.white('  npm install -g https://github.com/NU-AquaLab/thealeph-mcp.git\n'));
    ...
    throw new Error('The Aleph MCP not found. Please install it globally first.');
  }
  ...
  async verifyInstallation(mcpPath) {
    console.log(chalk.blue('\n🔍 Verifying The Aleph MCP installation...'));
  ...
  async setupClaudeDesktop() {
    console.log(chalk.blue.bold('🔧 The Aleph MCP Setup for Claude Desktop\n'));

    // Check if package is installed and get its path
    let mcpCommand;
    try {
      mcpCommand = this.getTheAlephMCPPath();
    } catch (error) {
      console.log(chalk.red.bold('\n❌ Setup cannot continue without The Aleph MCP installed.'));
      process.exit(1);
    }
    ...
    // Check if The Aleph is already configured
    if (config.mcpServers.thealeph) {
      console.log(chalk.yellow('⚠️  The Aleph MCP is already configured in Claude Desktop.'));
    ...
    const { apiUrl } = await inquirer.prompt([
      {
        type: 'list',
        name: 'apiUrl',
        message: 'Which The Aleph API would you like to use?',
        choices: [
          {
            name: 'Default (https://thealeph.ai)',
            value: 'https://thealeph.ai'
          },
          {
            name: 'Custom URL',
            value: 'custom'
      ...
      const { customUrl } = await inquirer.prompt([
        {
          type: 'input',
          name: 'customUrl',
          message: 'Enter the custom The Aleph API URL:',
          validate: (input) => {
            try {
              new URL(input);
              return true;
    ...
    // Configure The Aleph MCP
    config.mcpServers.thealeph = {
      command: mcpCommand
    };

    // Add environment variables if using non-default API
    if (finalApiUrl !== 'https://thealeph.ai') {
      config.mcpServers.thealeph.env = {
        THEALEPH_API_URL: finalApiUrl
      };
    }
    ...
    try {
      ...
      console.log(chalk.white('2. The Aleph tools will be available in your next conversation'));
      console.log(chalk.white('3. Try asking: "What tools do you have from The Aleph?"'));
      
      console.log(chalk.gray('\n💡 Tip: You can run "thealeph-mcp --test" to verify API connectivity'));
      ...
      console.log(chalk.gray(JSON.stringify({
        mcpServers: {
          thealeph: config.mcpServers.thealeph
        }
      }, null, 2)));
    }
  }
  ...
  async showCurrentConfig() {
    ...
    if (!configPath) {
      ...
      console.log(chalk.blue('💡 Run "thealeph-mcp --setup" to create one.'));
      return;
    }
    ...
    if (!config.mcpServers || !config.mcpServers.thealeph) {
      console.log(chalk.yellow('⚠️  The Aleph MCP is not configured.'));
      console.log(chalk.blue('💡 Run "thealeph-mcp --setup" to configure it.'));
      return;
    }
    
    console.log(chalk.green('✅ The Aleph MCP is configured:'));
    console.log(chalk.gray(JSON.stringify(config.mcpServers.thealeph, null, 2)));
  }
}
